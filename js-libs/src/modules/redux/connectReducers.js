...
export class Module {
   ...
   *   const out = nsSelector(metaSel, 'a_value', 'module.Module')
   *```
   *
   * @method
   * @private
   ...
   */
  _loadMetaSelectors = (path) => {
    const selectors = (
      path === ''
        ? this.module.metaSelectors
        : _.get(this.module.metaSelectors, path, {})
    );
    _.forIn(selectors, (selector, selPath) => {
      const isFn = typeof selector.fn === 'function';
      if (isFn) {
        this._nsMetaSelector(selector, selPath, path);
      }
      else {
        this._loadMetaSelectors(join(path, selPath));
      }
    });
  }
   ...
   * Example usage:
   *   ```
   *   // module.Module.metaSelectors = { a_value: {...} }
   *   const out = nsMetaSelector(metaSel, 'a_value', 'module.Module')
   *   ```
   *
   * @method
   * @private
   ...
   */
  _nsMetaSelector = (selector, selPath, path) => {
    let { selectors, fn } = selector;
    const nsPath = this._mkPath(path);
    selectors = selectors.map(sel => (state, ...args) => sel(_.get(state, nsPath), ...args));
    _.set(
      this.store.selectors,
      this._mkPath(path, selPath),
